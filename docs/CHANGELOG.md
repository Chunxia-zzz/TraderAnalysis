...
- **交易信号页 · 主跌浪超卖**：每只标的加 3 项 check（① 60日跌幅<30% 过滤烂票 ② 晨星低估 ③ 评分≥80 STRONG_BUY 确认），徽章显示通过数（绿 3/3 / 黄 2/3 / 灰），行内直接显示跌幅%与低估/溢价%数值
- 后端 `/api/scores/overview`：批量拉 60 根日线，新增 `dd60_pct/dd60_ok`（60日跌幅）、`ms_discount_pct/ms_undervalued`（晨星折价）
- 依据回测：RSI<25 反弹率 92-93%，叠加"排除 60 日跌>30%"可将最惨续跌从 -56% 收窄至 -33%，反弹率提至 94.7%（详见日志）

---

## [2026-08-26]

### 新功能 — 实时持仓与目标仓位（永久投资组合）

- **实时持仓查询**（`position_fetcher.py` + `GET /api/position`）：调用富途 OpenD 实时刷新实盘/模拟盘持仓（`accinfo_query` 资产 + `position_list_query(refresh_cache)` 持仓），支持本币/美元双计价，返回现金占比与每只标的市值占比
- **目标仓位（永久投资组合）**（`allocation.py` + `GET/PUT /api/allocation`）：美股 70%（大科技 28 + 半导体 21 + 灵活板块 21）/ 黄金 10%（GDX 为主）/ 比特币 20%（IBIT:MSTR:CRCL = 2:4:4），目标 vs 实际持仓对比，结合评分/RSI/晨星折价生成四级加仓建议（强烈建议加仓 / 建议加仓 / 等回调 / 高于公允价值回调再买）
- **分批加仓计划**：黄金/比特币按现货价格区间（ETF↔期货两点线性校准）分 3 批（底部区买 50% / 中部 30% / 顶部 20%），当前价低于批次顶部即可执行
- **目标配置可编辑**：`data/target_allocation.json` 存储，页面在线调整权重/现货区间/校准点
- **标的管理**：移除 ZURVY（富途不提供美股 OTC 行情，无法拉取数据）
- **前端**：新增「我的持仓」「目标仓位」页面，导航整合为「持仓配置」下拉；市场复盘打 beta 标签

### 说明

- `/api/position`、`/api/allocation` 依赖本机富途 OpenD（127.0.0.1:11111），仅本地开发环境可用；线上服务器返回引导提示
- 实盘账户查询需 `security_firm=FUTUSECURITIES` 且持仓需 `refresh_cache=True` 才有实时市值
